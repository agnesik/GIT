...
else
{
    ...
    $sql = "SELECT * FROM `student` WHERE user='$login' AND pass='$haslo'"
    ;
//wybierz z tabeli ... zmienne php bedace lancuchami zamykamy w apostrofach
    ...
    if ($rezultat = @$polaczenie->query($sql))   //bedziemy wyciagac dane z tabeli, jesli bedzie blad przyjmie wartosc false
    {
        ...
        if($ilu_userow>0)
        {
            $_SESSION['zalogowany']=true; //flaga-zmienna ktora ustawiamy jako symbol zajscia czegos w kodzie, umozliwia przekazywanie zmiennych pomiedzy stronami z uzyciem globalnej tablicy asocjacyjnej
            ...
            $_SESSION['rok'] = $wiersz['rok_id'];

            unset($_SESSION['blad']); //jesli udalo nam sie zalogowac to usuniemy z sesji zmienna blad, unset - wywal, usun

            $rezultat->close();   //zwolni pamiec zeby pozbyc sie niepotrzebnych rezultatow zapytania
            header('Location:student.php'); //pobrane dane chcemy wyswietlic w pliku stunt.php - przekierowujemy

        }else {   //nie ma nikogo w bazie o tym loginie i hasle, gdy poda nieprawidlowe dane,

            $_SESSION['blad'] = '<span style="color:red">Nieprawidłowy login lub hasło!</span>'; //ustawiamy info o bledzie i przenosimy do forumlarza logowania
